...
flags.DEFINE_integer('arch',  3, '网络架构：1，仅fm；2，cnn；3，deepFM')
flags.DEFINE_float('lr', 0.0001, '设置学习率')
flags.DEFINE_float('pos_weight', 1, '惩罚FN')
flags.DEFINE_integer('data_source', 3, '数据源：1，cnt 59维交叉特征；2，cnt 66维交叉特征；3，cnt 78维交叉特征')
...
flags.DEFINE_string('logdir', 'df_cnt_78_', '日志保存路径')



flags.DEFINE_list('hidden', [32, 16, 8], '设置隐藏层结构')
...
def build_graph(train, hidden):
    with tf.name_scope('Input'):
        cnt_i = tf.placeholder(dtype=tf.float32, shape=[None, cfg.feature_len])
    ...
    # 构造网络结构
    summary = []
    logits, outputs = build_arch(cnt_i, hidden, summary, is_train if train else False)
    ...
    if train:
        return cnt_i, labels, is_train, loss, outputs, merged_summary, opt, init_op, saver
    else:
        return cnt_i, init_op, saver, outputs
...
def build_arch(cnt_feature, hide_layer, summary, is_training):
    if cfg.arch == 1 or cfg.arch == 3:
        ...
        with tf.variable_scope('FM'):
            ...
            linear_terms = tf.matmul(cnt_feature, w)
            linear_terms = tf.add(linear_terms, b, name='linear')
            summary.append(tf.summary.histogram('linear_terms', linear_terms))

            part1 = tf.square(tf.matmul(cnt_feature, v))
            part2 = tf.matmul(tf.square(cnt_feature), tf.square(v))
