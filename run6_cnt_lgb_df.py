...
flags.DEFINE_integer('data_source', 3, '数据源：1，cnt 59维交叉特征；2，cnt 66维交叉特征；3，cnt 78维')
...
flags.DEFINE_string('logdir', 'df_cnt_78_9098_', '日志保存路径')
...
def read_data(dir, cls):
    ...
    print('x2:', directory + X2_file)
    X1 = feather.read_dataframe(directory + X1_file).values
    X2 = sparse.load_npz(directory + X2_file)
...
def get_test_data(test_set):
    ...
    if test_set == 1:
        test_x_1,test_x_2, _ = read_data('test', 'test1')
    elif test_set == 2:
        test_x_1,test_x_2, _ = read_data('test', 'test2')
    else:
        return None
    return test_x_1,test_x_2


def get_valid_data(fold):
    print('reading valid data from fold', fold, '...')
    if fold == 1:
        val_x1, val_x2, val_y = read_data('tr1', 'tr')
    elif fold == 2:
        val_x1, val_x2, val_y = read_data('tr2', 'tr')
    elif fold == 3:
        val_x1, val_x2, val_y = read_data('tr3', 'tr')
    elif fold == 4:
        val_x1, val_x2, val_y = read_data('tr4', 'tr')
    else:
        return None
    print('x1 shape:', val_x1.shape)
    print('x2 shape:', val_x2.shape)
    return val_x1,val_x2, val_y, val_y.shape[0]
...
def get_train_data(fold, stage=0):
    ...
    if fold == 1:
        ...
        return None
    tr_x1, tr_x2, tr_y = read_data(tr[stage], 'tr')
    print('x1 shape:', tr_x1.shape)
    print('x2 shape:', tr_x2.shape)
    return tr_x1, tr_x2, tr_y, tr_y.shape[0]
    ...
    with graph.as_default():
        ...
        with tf.Session() as sess:
            ...
            for e in range(last_epoch, epoch):
                ...
                if cfg.valid:
                    val_x1, val_x2, val_y, val_size = get_valid_data(fold)
                for stage in range(last_stage, 3):
                    print()
                    print('stage', stage, ':')
                    tr_x1, tr_x2, tr_y, tr_size = get_train_data(fold, stage)
                    ...
                    for i in bar:
                        if i == train_batch:
                            ...
                            tr_x2_batch = tr_x2[tr_idx:].tocoo()
                            tr_y_batch = tr_y[tr_idx:]
                            tr_idx = tr_size-1
                        else:
                            tr_x1_batch = tr_x1[tr_idx: tr_idx+cfg.batch] / 255.
                            tr_x2_batch = tr_x2[tr_idx: tr_idx+cfg.batch].tocoo()
                            tr_y_batch = tr_y[tr_idx: tr_idx+cfg.batch]
                            tr_idx += cfg.batch
                        tr_indics = np.mat([tr_x2_batch.row, tr_x2_batch.col]).transpose()
                        tr_values = tr_x2_batch.data
                        tr_dense_shapes = tr_x2_batch.shape
                        ...
                        if i % cfg.summary == 0:
                            ...
                            if cfg.valid:
                                ...
                                val_x2_batch = val_x2[val_idx: val_idx + cfg.batch].tocoo()
                                val_y_batch = val_y[val_idx: val_idx + cfg.batch]
                                val_idx += cfg.batch
                                val_indics = np.mat([val_x2_batch.row, val_x2_batch.col]).transpose()
                                val_values = val_x2_batch.data
                                val_dense_shapes = val_x2_batch.shape
                    ...
                    del tr_x1, tr_x2, tr_y
                    print()
                    print('stage', stage, 'finished!')
                if cfg.valid:
                    del val_x1, val_x2, val_y
...
def evaluate(graph, valid, logdir, fold):
    ...
    if valid:        # 读取数据
        ...
        X1_test2, X2_test2 = get_test_data(2)
    ...
    with graph.as_default():
        ...
        with tf.Session() as sess:
            ...
            if valid:
                ...
                result_test2 = predict(sess, batch_num_test2, result_test2, data_size_test2, (X1_test2, X2_test2), outputs,
                                       indices_i, values_i, dense_shape_i, ctr_i)
                print()
                print('result_test2 shape:', result_test2.shape)

                del X1_test2, X2_test2
...
def predict(sess, batch_num, result, data_size, X, outputs, indices_i, values_i, dense_shape_i, ctr_i):
    ...
    for i in bar:
        if i == batch_num:
            x1_batch = X1[test_idx:] / 255.
            x2_batch = X2[test_idx:].tocoo()
        else:
            x1_batch = X1[test_idx: test_idx + cfg.batch] / 255.
            x2_batch = X2[test_idx: test_idx + cfg.batch].tocoo()
        indics = np.mat([x2_batch.row, x2_batch.col]).transpose()
        values = x2_batch.data
        dense_shapes = x2_batch.shape
...
def build_arch(feature, ctr_feature, hide_layer, summary, is_training):
    with tf.name_scope('arch'):
        ...
        with tf.variable_scope('FM'):
            ...
            # ctr_in = tf.layers.batch_normalization(ctr_feature, training=is_training, name='ctr_in')
            # cnt_layer = tf.layers.dense(ctr_feature, cfg.cnt_layer, activation=tf.nn.relu)
        ...
        cnt_layer = ctr_feature
        for i, num in enumerate(cfg.cnt_layer):
            cnt_layer = tf.layers.dense(cnt_layer, num, activation=None, use_bias=True, name='layer' + str(i + 1))
            cnt_layer = tf.layers.batch_normalization(cnt_layer, training=is_training, name='bn_layer' + str(i + 1))
            cnt_layer = tf.nn.relu(cnt_layer, name='act_layer' + str(i + 1))
        y_cnt = tf.layers.dense(cnt_layer, 1, activation=None, use_bias=True)

        # logits = 0.3*y_cnt + 0.7*y_dnn
        logits = tf.layers.dense(tf.concat([y_cnt, y_dnn], 1), 1, activation=None)
