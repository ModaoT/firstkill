...
#    hyper parameters      #
############################
flags.DEFINE_string('mode', 'test', 'test:仅执行一次操作；all：一个完整的4stack；tv：train+valid')
...
flags.DEFINE_integer('data_source', 1, '数据源：1，cnt 78维交叉特征+13475；2，全部维度')
...
flags.DEFINE_integer('feature_len', 13475, '特征维度')
...
flags.DEFINE_boolean('valid', True, '是否在训练中做交叉验证')  # 在train为False的前提下，valid如果为False，则生成submission，valid为True，在验证集上评估auc
flags.DEFINE_string('logdir', 'df_cnt_13475_', '日志保存路径')
                    ...
                    for i in bar:
                        ...
                        if i % cfg.summary == 0:
                            ...
                            if cfg.valid:
                                ...
                                val_x2_batch = val_x[val_idx: val_idx + cfg.batch].tocoo()
    ...
    with graph.as_default():
        ...
        with tf.Session() as sess:
            ...
            if valid:
                ...
                result_valid = predict(sess, batch_num_valid, result_valid, data_size_valid, X1_valid, outputs,
...
def build_arch(feature, hide_layer, summary, is_training):
    with tf.name_scope('arch'):
        fm_input_len = cfg.feature_len
        if cfg.arch == 1 or cfg.arch == 3:
            v1 = tf.Variable(tf.truncated_normal(shape=[fm_input_len, cfg.embed], stddev=0.01),
                            dtype=tf.float32, name='v1')
            v2 = tf.Variable(tf.truncated_normal(shape=[fm_input_len, cfg.embed], stddev=0.01),
                             dtype=tf.float32, name='v2')

            with tf.variable_scope('FM'):
                b = tf.get_variable('bias', shape=[1], initializer=tf.zeros_initializer())
                w = tf.get_variable('w', shape=[fm_input_len, 1],
                ...
                part1_1 = tf.square(tf.sparse_tensor_dense_matmul(feature, v1))
                part2_1 = tf.sparse_tensor_dense_matmul(tf.square(feature), tf.square(v1))
                part1_2 = tf.square(tf.sparse_tensor_dense_matmul(feature, v2))
                part2_2 = tf.sparse_tensor_dense_matmul(tf.square(feature), tf.square(v2))

                interaction_terms1 = tf.multiply(0.5,
                                                tf.reduce_mean(tf.subtract(part1_1, part2_1), 1, keepdims=True), name='interaction1')
                interaction_terms2 = tf.multiply(0.5,
                                                 tf.reduce_mean(tf.subtract(part1_2, part2_2), 1, keepdims=True),
                                                 name='interaction2')
                summary.append(tf.summary.histogram('interaction_terms1', interaction_terms1))
                summary.append(tf.summary.histogram('interaction_terms2', interaction_terms2))

                # y_fm = tf.add(linear_terms, interaction_terms, name='fm_out')
                y_fm = tf.concat([linear_terms, interaction_terms1, interaction_terms2], axis=1, name='fm_out')
                y_fm = tf.layers.dense(y_fm, 1, activation=None)
        ...
        if cfg.arch == 2 or cfg.arch == 3:
            with tf.variable_scope('DNN', reuse=False):
                # embedding layer
                dnn_v = tf.Variable(
                    tf.truncated_normal(shape=[fm_input_len, cfg.embed], mean=0, stddev=0.01),
                    dtype='float32')
